...
description: "Secure handling of tokens and API credentials in KnowledgeX."
icon: 'key'
---

# Encryption & Credentials

KnowledgeX ensures that **tokens** (user OAuth tokens) and **API credentials** (e.g., client secrets) are stored in an **encrypted** format—minimizing risk even if the database is compromised.

<Info>
**Why Encryption Matters**
Storing tokens in plaintext is a major security risk. KnowledgeX uses an **encryption key** to scramble all sensitive fields, preventing unauthorized use or exposure.
</Info>
...
<AccordionGroup>
  ...
  **Secure Storage**  
  ...
  All KnowledgeX endpoints must be accessed via HTTPS. This ensures that data in transit (including tokens) isn’t exposed to eavesdroppers or man-in-the-middle attacks.
  
  - **Custom SSL**: For self-hosted setups, add an SSL certificate to your server or load balancer.
  - **Cloud**: KnowledgeX Cloud enforces TLS by default.
  ...
  **Rotation**  
  ...
  - KnowledgeX provides a tool/CLI to re-encrypt existing records with a new key.
