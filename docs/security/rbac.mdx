...
description: "How KnowledgeX handles role-based access control and compliance."
icon: 'user-shield'
---
...
<Callout type="info">
KnowledgeX leverages <Tooltip text="RBAC stands for Role-Based Access Control">RBAC</Tooltip> to ensure users only interact with data and configurations they’re allowed to see.
</Callout>
...
<CardGroup>
  <Card title="Data Minimization">
    KnowledgeX only stores file contents when ingestion is explicitly enabled. Otherwise, it retains only references or metadata about files unless a local ingestion pipeline is used for transformation.
  </Card>
